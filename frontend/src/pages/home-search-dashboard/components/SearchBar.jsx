...
const SearchBar = ({ onSearch, onVoiceSearch, recentSearches, onClearRecentSearch }) => {const { t } = useTranslation();
  ...
  return (
   ...
   {showSuggestions && suggestions?.length > 0 &&
     ...
     <div className="p-2">
      <div className="text-xs text-muted-foreground px-3 py-2 font-medium">{t("homeSearchDashboard.suggestions")}</div>
   ...
   {recentSearches?.length > 0 && !showSuggestions && !searchQuery &&
      <div className="mt-4">
     <div className="text-sm text-muted-foreground mb-2 font-medium">{t("homeSearchDashboard.recentSearches")}</div>
   ...
   {isListening &&
     ...
     <div className="flex items-center justify-center space-x-2 text-error">
      ...
      <span className="text-sm font-medium">{t("homeSearchDashboard.listening")}</span>
     </div>
     <p className="text-xs text-muted-foreground mt-1">{t("homeSearchDashboard.speakNowToSearch")}</p>
    </div>
      }
  </div>);
